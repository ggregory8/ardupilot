...
void Copter::custom_led()
{
    bool ant_aux_mode_enable = true;

    // Update AUX pin for PWM Output
    switch(g.ev_ant_aux_pin) {
        case 0:
            // If EV_ANT_AUX_PIN = 0 then disable LED/Antenna functionality
            ant_aux_mode_enable =  false;
        case 1:
            // AUX 1 = pin 9
            ant_pwm_aux_pin = 9;
            break;
    ...
    if (g.ev_ant_msg >= 2) {
        ...
        {
            if (ant_aux_mode_enable) { 
                gcs_send_text_fmt(MAV_SEVERITY_INFO, "LED Enabled hb: %d yaw: %d mav_home: %d", int16_t(rtm_bearing/100), int16_t(ahrs.yaw_sensor/100), int16_t(mav_to_rtm_bearing));
            ...
            // gcs_send_text_fmt(MAV_SEVERITY_INFO, "EV_ANT_AUX_PIN: %d = pin: %d", int16_t(g.ev_ant_msg), ant_pwm_aux_pin);
                ...
                gcs_send_text_fmt(MAV_SEVERITY_INFO, "LED Disabled hb: %d yaw: %d mav_home: %d", int16_t(rtm_bearing/100), int16_t(ahrs.yaw_sensor/100), int16_t(mav_to_rtm_bearing));
            }
            update_count_led = 0;
        }
    }
...
void Copter::batt_consumption()
{
    ...
    batt_cap_remaining = battery.pack_capacity_mah() - battery.current_total_mah();
    ...
    */
    ...
    if (update_count == 200)
        gcs_send_text_fmt(MAV_SEVERITY_INFO, "Batt remaining: %.0f of %.0fmah, %.0f, ttf: %ds", batt_cap_remaining, battery.pack_capacity_mah(), (batt_cap_remaining - required_ma_to_home),time_to_fs);
